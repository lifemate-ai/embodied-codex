...
|----------|---------|-------------|
| `MEMORY_DB_PATH` | `~/.codex/memories/memory.db` | SQLite database path |

## Migrating from ChromaDB

If you have existing memories in ChromaDB (older versions used `~/.codex/memories/chroma`), run the migration script:
...
uv run python scripts/migrate_chroma_to_sqlite.py \
    --source ~/.codex/memories/chroma \
    --dest ~/.codex/memories/memory.db
