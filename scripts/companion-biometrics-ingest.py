...
def build_handler_class(
    output_path: Path,
    bearer_token: str = "",
    default_source: str = "health-connect",
    ...
    return type(
        ...
        (CompanionIngestHandler,),
        {
            ...
            "default_source": default_source.strip() or "health-connect",
        },
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--bind",
        default=os.getenv("CODEX_COMPANION_BIOMETRICS_INGEST_BIND", "0.0.0.0"),
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("CODEX_COMPANION_BIOMETRICS_INGEST_PORT", "8765")),
    )
    parser.add_argument(
        "--output",
        default=os.getenv("CODEX_COMPANION_BIOMETRICS_PATH", "/tmp/companion_biometrics.json"),
    )
    parser.add_argument(
        "--token",
        default=os.getenv("CODEX_COMPANION_BIOMETRICS_INGEST_TOKEN", ""),
    )
    parser.add_argument(
        "--source",
        default=os.getenv("CODEX_COMPANION_BIOMETRICS_SOURCE", "health-connect"),
    )
    return parser
