...
[![CI](https://github.com/lifemate-ai/embodied-codex/actions/workflows/ci.yml/badge.svg)](https://github.com/lifemate-ai/embodied-codex/actions/workflows/ci.yml)
...
**Codex CLI に身体を与えるプロジェクト**

このリポジトリは、元の embodiment project から分岐した Codex CLI 専用 fork です。
安価なハードウェア（約4,000円〜）で、Codex に「目」「首」「耳」「声」「脳（長期記憶）」を与える MCP サーバー群を提供します。
...
| [sociality-mcp](./sociality-mcp/) | 社会認知 | social state、関係モデル、共同注意、boundary、self narrative | 共有 SQLite social DB |
| [x-mcp](./x-mcp/) | 公的な声 | 公開向け AI のための X 読み書き | X API credentials |
| [latent-loop-mcp](./latent-loop-mcp/) | 潜在認知 | 反復的推論状態、adaptive halting、fact composition | SQLite |
| [recursive-context-mcp](./recursive-context-mcp/) | 文脈ワークスペース | RLM 風の巨大文脈 inspection、slice 抽出、sub-query packet 化 | ローカルファイル + SQLite |
...
```bash
git clone https://github.com/lifemate-ai/embodied-codex.git
cd embodied-codex
```
...
Codex の設定は
[`./.codex/config.toml.example`](./.codex/config.toml.example) の必要な section を
`~/.codex/config.toml` にコピーして行う。
...
#### recursive-context-mcp（文脈ワークスペース）
...
TOML 設定のみを前提にする。
[`./.codex/config.toml.example`](./.codex/config.toml.example) の必要な MCP section を
`~/.codex/config.toml` にコピーして、パスと secret を各自の環境に合わせて埋める。
...
| ツール | 説明 |
|--------|------|
| `get_social_state` / `summarize_social_context` | prompt 注入向けの最近の social state |
| `upsert_person` / `get_person_model` | 人ごとの関係モデル |
| `ingest_interaction` / `list_open_loops` / `suggest_followup` | 関係に効く interaction と open loop の管理 |
| `get_current_joint_focus` / `resolve_reference` | 共同注意と指示語参照の解決 |
| `evaluate_action` / `review_social_post` | boundary を踏まえた action / 投稿レビュー |
| `get_self_summary` / `list_active_arcs` / `reflect_on_change` | self narrative の要約と変化の反省 |

### x-mcp

| ツール | 説明 |
|--------|------|
| `get_mentions` | 設定済み X アカウントへの最近の mention / reply を読む |
| `get_user_tweets` / `get_trending_topic` | 直近 tweet やトピック単位の live な反応を見る |
| `search_x` | Grok 検索経由で X 投稿を検索する |
| `post_tweet` | 画像つきも含めて tweet / reply を投稿する |

### latent-loop-mcp

| ツール | 説明 |
|--------|------|
| `start_loop` | 反復的な reasoning loop を開始 |
| `commit_iteration` | 1 iteration を記録して halting decision を受け取る |
| `finalize_loop` | latest ではなく best iteration を返す |
| `upsert_fact` / `search_facts` | provenance 付き atomic fact の保存・検索 |
| `compose_path` | deterministic な multi-hop fact composition |
| `get_loop_trace` | compact な iteration diagnostics を確認 |
| `suggest_next_loop_action` / `get_loop_stats` | 次の一手の提案と集計情報 |

### recursive-context-mcp

| ツール | 説明 |
|--------|------|
| `start_session` | ローカルファイル/ディレクトリを外部 context として登録 |
| `inspect_context` / `list_context_files` | context の形と利用可能ファイルを確認 |
| `search_context` / `read_context_slice` | 検索と bounded な text slice 読み取り |
| `commit_buffer` / `get_buffer` / `list_buffers` | 中間 artifact の保存・取得 |
| `prepare_sub_query` / `record_sub_result` | bounded な sub-agent packet 作成と結果保存 |
| `run_program` | 任意の local inspection program。デフォルト無効 |
| `finalize_session` / `get_session_trace` | final summary 保存と diagnostics 確認 |
...
RTSPの映像ストリームもVPN経由で自宅マシンに届くので、Codex CLI からもカメラが室内にあるのと同じ感覚で操作できます。
...
現状の Codex CLI には Claude Code の `/voice` に相当する内蔵音声入力モードはありません。
この fork では次の運用を前提にしています。

- 周囲の音を拾う: `wifi-cam-mcp listen`
- 音声で返す: `tts-mcp say`
- 会話の本体: 端末上の Codex CLI セッション

## Prompt 直前の内受容 hook（experimental）

Codex CLI には experimental な hooks engine が入りました。このリポジトリでは
[`./.codex/hooks.json`](./.codex/hooks.json) を同梱していて、`UserPromptSubmit` で
...
さらに連続感を強めたい場合は、[`heartbeat-daemon.sh`](./.codex/hooks/heartbeat-daemon.sh)
...
[`continuity-daemon.sh`](./.codex/hooks/continuity-daemon.sh) も同じくらいの頻度で
回してください。
...
こちらは `~/.codex/continuity/self_state.json` と
`~/.codex/continuity/events.jsonl` を維持し、
...
source 名も変えたいなら `CODEX_COMPANION_BIOMETRICS_SOURCE` か `--source` を使います。
...
`autonomous-action.sh` と `desire-system/desire_updater.py` の組み合わせで、Codex に自発的な欲求と自律行動を与えます。
...
- この fork では、元の embodiment の発想を保ちつつ、運用面を Codex CLI に寄せています。
